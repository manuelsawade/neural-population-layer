...
input_position_1 = 0.1  # Original input value
input_position_2 = 0.01  # Perturbed input value
sigma = 1.0
...
axes[0, 2].plot(x_smooth, activation_1_smooth, color='gray', linewidth=2)
axes[0, 2].plot(x_smooth, activation_2_smooth, color='black', linewidth=2)
axes[0, 2].axvline(decoded_value1, color='gray', linestyle='--', label=f"Original = {decoded_value1:.4f}")
axes[0, 2].axvline(decoded_value2, color='black', linestyle='--', label=f"Perturbed = {decoded_value2:.4f}")
axes[0, 2].set_title("Weighted Average Decoder")
...
# --- Plot 3 (Heatmap) ---
activation_combined = (activation_1 + activation_2) / 2  # Combined activation for the decoder
# axes[1, 2].set_yticks([])
# axes[1, 2].set_xlabel("Input / Hidden Units")
# axes[1, 2].set_xlim(orientations[0], orientations[1])
# axes[1, 2].grid(False)
axes[1, 2].set_visible(False)
# axes[1, 2].set_xticks([])
# axes[1, 2].set_yticks([])
# axes[1, 2].text(-1, 2, "Difference")

# Adjust layout and save
plt.tight_layout(h_pad=0.3, w_pad=0.4)
plt.savefig("tuning_curves_with_decoder_perturbed.png", dpi=300)
plt.close(fig)
