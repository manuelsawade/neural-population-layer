...
def main():
    ...
    stacks = linear_stack
    metric_map = [["accuracy", "test_accuracy", "max"], ["loss", "test_loss", "center"], ["fsa_inf_mean", "test_fsa_inf_mean", "max"]]
    ...
    for ax, metrics in zip(axes, metric_map):
        ...
        for i, noise in enumerate([0.0, 0.5, 1.0]):
            ...
            if i == 2 and "accuracy" in metrics:
    ...
    # Layout and save
    plt.tight_layout()
    plt.savefig(f"{folder}{identifier}_tuning_performance_linear.png", dpi=300)
    plt.close(fig)
