...
ax2.plot(x.squeeze(), softmax.squeeze(), '-', color='purple', linewidth=1, label=rf'Softmax ($\alpha = {alpha}$)')
ax2.plot(x.squeeze(), gaussian.squeeze(), '-', color='black', linewidth=1, label=rf"Gaussian ($\sigma = {sigma}$)")
