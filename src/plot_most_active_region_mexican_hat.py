...
mexican_hat = (1 - ((x - max_x) / sigma) ** 2) * np.exp(-0.5 * ((x - max_x) / sigma) ** 2)
# Normalize tuning curve to input range (0 to max input)
mexican_hat = mexican_hat / np.max(mexican_hat) * np.max(input_values)
...
ax2.plot(x, mexican_hat, '-', color='gray', linewidth=2)
...
ax_heat = axes[1]
ax_heatmap = np.expand_dims(mexican_hat, axis=0)
