...
runs = ["linear", "population", "population_encoding", "softmax_gaussian", "preferred_value"]
...
def run(config, stack_name):
        stack = get_stack(config, stack_name)
...
def test(config, checkpoint_data, scope, stack):
    model = get_stack(config, stack)
...
def test_best_trial(result, metric, mode, stack, scope="last"):
    ...
    with best_checkpoint.as_directory() as checkpoint_dir:
        ...
        test(best_trial.config, best_checkpoint_data, scope, stack)   

def main(data_dir, stack):
    ...
    test_best_trial(result, metric=target_metric, stack=stack, mode="min")
